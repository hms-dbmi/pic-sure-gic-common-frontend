...
public class PicSureController {
    ...
    private <T> ResponseEntity<T> formatRequestAndRunPost(QueryRequest request, String relativePath, Class<T> returnType) {
        ...
        URI path = remoteResource.base().resolve(relativePath);
        QueryRequest chainRequest = new GeneralQueryRequest();
        chainRequest.setQuery(request.getQuery());
        chainRequest.setResourceCredentials(request.getResourceCredentials());
        chainRequest.setResourceUUID(remoteResource.remote());
        return http.post(path, chainRequest, returnType, HttpHeaders.AUTHORIZATION, BEARER + remoteResource.token())
